    ...
    <div className="min-h-screen bg-[#faf9f6]">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-[#1a1a2e]">Tilbud</h1>
          <Link href="/tilbud/ny" className="bg-[#1a1a2e] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#2d2d4a] transition">
            + Nyt tilbud
          </Link>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {(!offers || offers.length === 0) && (
                <tr><td colSpan={5} className="text-center py-8 text-gray-400">Ingen tilbud endnu</td></tr>
              )}
              {offers?.map((o) => (
                <tr key={o.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      ...
                      {o.status === "draft" ? "Kladde" :
                       o.status === "sent" ? "Sendt" :
                       o.status === "accepted" ? "Accepteret" :
                       o.status === "rejected" ? "Afvist" : o.status}
                    </span>
                  </td>
